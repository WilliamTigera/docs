...
# Integrate scanner into your build pipeline

## Big picture

Integrate the Image Assurance scanner into your build pipeline to ensure builds are checked by Image Assurance before deployment.
...
- [Download the scanner](#download-the-scanner)
...
- The CLI binary is available in the worker or runner where the pipeline is executing
...
- Create a secret containing the API-Token and make it available in the pipeline (for example, using a SECURE_API_TOKEN environment variable)
